...
```js
let iterator = async.iterator([
    function(){ sys.p('one'); },
    function(){ sys.p('two'); },
    function(){ sys.p('three'); }
]);

node> let iterator2 = iterator();
'one'
node> let iterator3 = iterator2();
'two'
node> iterator3();
'three'
node> let nextfn = iterator2.next();
node> nextfn();
'three'
```
...
```js
node> let fn = async.apply(sys.puts, 'one');
node> fn('two', 'three');
one
two
three
...
```js
let slow_fn = function (name, callback) {
    // do something
    callback(null, result);
};
let fn = async.memoize(slow_fn);
...
```js
let q = async.queue(async.asyncify(async function (file) {
  let intermediateStep = await processFile(file);
